...
def evaluate(model, iterator, threshold=None, test=False, epoch='', hp=None):
    ...
    if threshold is not None:
        ...
        if test:
            ...
            save_path = os.path.join(save_dir, f'epoch{epoch}_probabilities.csv')
        ...
        for th in tqdm(thresholds, total=len(thresholds), desc='Finding best threshold...'):
...
def train_step(train_iter, model, optimizer, scheduler, hp):
    ...
    for i, batch in enumerate(tqdm(train_iter, desc='Iterating through training batches...', total=len(train_iter))):
        optimizer.zero_grad()
...
def train(trainset, validset, testset, run_tag, hp):
    ...
    for epoch in range(1, hp.n_epochs+1):
        ...
        test_f1 = evaluate(model, test_iter, threshold=th, test=True, epoch=epoch, hp=hp)
        ...
        print(f"epoch {epoch}: train_f1={dev_f1:.4f}, test_f1={test_f1:.4f}, best_test_f1={best_test_f1:.4f}")
        print(f"\t(Time Taken - Train: {train_time}, Eval: {eval_time})")

        # logging
        scalars = {'f1': dev_f1,
                   't_f1': test_f1}
        writer.add_scalars(run_tag, scalars, epoch)
    print(f"\t(Total Time Taken - Train: {total_train_time}, Eval: {total_eval_time})")
