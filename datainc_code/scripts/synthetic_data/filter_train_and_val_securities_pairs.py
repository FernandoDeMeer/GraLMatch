...
def filter_pairs_no_data_drift(pairs_df, value_df, map_df, syn_records_dict, pairs_to_filter):
    ...
    for index, pair in tqdm(pairs_df[:pairs_to_filter].iterrows(), total= pairs_to_filter,
        ...
        if '_' in pair_gen_id:
            dict_gen_id = pair_gen_id.split('_', 1)[0]
            syn_dict = syn_records_dict[int(dict_gen_id)]
        else:
            syn_dict = syn_records_dict[int(pair_gen_id)]
...
def main():
    ...
    securities_df = pd.read_csv(securities_df_path, low_memory=False)
    ...
    syn_pairs_data_path = os.path.join(syn_data_path, 'securities')
    ...
    checked_train_pairs = filter_pairs_no_data_drift(syn_pairs_train, securities_df, securities_mapping_df, syn_records_dicts, 10000)
    checked_train_pairs.to_csv(os.path.join(syn_pairs_data_path, 'filtered_train.csv'))

    checked_val_pairs = filter_pairs_no_data_drift(syn_pairs_val, securities_df, securities_mapping_df, syn_records_dicts, 5000)
